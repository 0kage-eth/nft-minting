...
const mintNFTs = async (hre: HardhatRuntimeEnvironment) => {
    ...
    console.log(
        `Randomized NFT Token URI is ${await randomizedNFTContract.tokenURI(randomizedNFTTokenId)}`
    )
    ...
    console.log(`Dynamic NFT Token URI is ${await dynamicNFTContract.tokenURI(dynamicNFTTokenId)}`)
}
